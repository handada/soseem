...
{
    // 패싯의 검색 엘리먼트 액션
    $('#ec-product-searchdata-form .xans-product-searchfilterlist .ec-product-searchdata-form').click(function()
    {
        if (EC_FRONT_PRODUCT_SEARCH_DATA.setFacetSelector($(this)) === false) {
            return false;
        }
     ...
     // 버튼식 금액 찾기
    $('.xans-product-searchfilterlist .ec-product-searchdata-price').click(function()
    {
    ...
    $('#ec-product-searchdata-mobile-button').click(function(){
        ...
        $('.xans-product-searchfilterlist').each(function() {
            ...
            var iWithin = 0;
            var sWithin = $(this).find('.withinInput input[type="text"]').val();
    ...
    $('.xans-product-searchdata .btnClose').click(function(){
        $('#ec-product-searchdata-form').hide();
        $('#container').css('height','');
        $('#ec-product-searchdata-form').find('.xans-product-searchdata.typeSlide').removeClass('open');

        EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM.setLayerMoreFilter(true);
    });

    // form input에 걸려있는 submit 기본 이벤트 제거
    $('#ec-product-searchdata-form .withinInput input[type="text"]').live('keyup keypress', function(event) {
        if (event.keyCode === 13) {
    ...
    $('.btnSearchDelete').live('click', function() {
        ...
        if (sStype === 'ePriceSearch') {
            $('#ec-product-searchdata-form #ec-product-searchdata-minval').val(0);
            $('#ec-product-searchdata-form #ec-product-searchdata-maxval').val(0);
            ...
            $('#ec-product-searchdata-form .xans-product-searchfilterlist .withinInput input[type="text"]').val('');
        } else {
            $('#ec-product-searchdata-form .ec-product-searchdata-form.ec_search_selected').each(function() {
            ...
            $('#ec-product-searchdata-form .ec-product-searchdata-form:checked').each(function() {
                if (sSvalue === $(this).val()) {
                    $(this).removeClass('ec_search_selected');
                    $(this).attr('checked',false);
                    return false;
    ...
    $('.ec-product-searchdata-reset').click(function() {
        ...
        $('#ec-product-searchdata-form #ec_mobile_brand_0').removeClass('checked');
        $('#ec-product-searchdata-form #ec_mobile_price_0').removeClass('checked');

        $('#ec-product-searchdata-form #ec-product-searchdata-minval').val('');
        $('#ec-product-searchdata-form #ec-product-searchdata-maxval').val('');
        EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMinPrice = 0;
        EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMaxPrice = 0;
    ...
    $('#ec-product-searchdata-minval , #ec-product-searchdata-maxval').blur(function(){
        ...
        if ($(this).attr('id') === 'ec-product-searchdata-minval') {
            EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMinPrice = $(this).val();
        } else {
            EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMaxPrice = $(this).val();
        }

        $(this).val(SHOP_PRICE.toShopPrice($(this).val(), false));
    });
    ...
    // 결과 내 검색 Enter 이벤트
    $('.xans-product-searchfilterlist .withinInput input.keyword').keypress(function(e) {
        if (e.keyCode === 13 && $(this).attr('onkeyup') !== '') {
            $('.withinInput button.btnResearch').click();
        }
    });

    // 결과 내 검색 자동완성 레이어 클릭 처리
    $('.xans-product-searchfilterlist .suggest li').live('click', function() {
        $(this).closest('.withinInput').find('input[type="text"]').val($(this).text());
    ...
    $('.withinInput button.btnResearch').click(function() {

        var sWithin = $(this).closest('.withinInput').find('input[type="text"]').val();

        if (typeof(sWithin) !== 'undefined') {
            // 앞/뒤 공백 및 멀티 스페이스 제거
            sWithin = $.trim(sWithin.replace(/\s{2,}/g, ' '));

            if (sWithin !== '') {
                // 실제로 값이 존재하는 경우에 처리
                // 셀럭터는 btnResearch + withinInput 등이 세트로 묶여있기 때문에 별도의 ID로 지정할 필요 없음
                $(this).closest('.withinInput').find('input[type="text"]').val(sWithin);
...
var EC_FRONT_PRODUCT_SEARCH_DATA =
{
    ...
    setSearchOptionButton : function()
    {
        if (this.getCommonObjectValue('V') === 6) {
            ...
            $('#ec-product-searchoption').remove(); // 검색옵션 레이어
            $('#ec-product-searchdata-searchkeyword_form').find('.btnSearchOption').remove(); // 검색옵션 버튼
        } else {
    ...
    setSearchinit : function() {
        ...
        if (EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM.isIncludeFixedForm() === true) {
            ...
            $('.xans-product-searchdata').find('button.btnDelete').live('click', function() {
                if ($(this).closest('.withinInput').length === 0) {
            ...
            $('.xans-product-searchfilterlist .withinInput .btnDelete').click(function() {
                $(this).closest('.withinInput').find('input[type="text"]').val('').focus();
                $(this).closest('.xans-product-searchfilterlist').removeClass('checked');
            });
        ...
        $('.ec-product-searchdata-form-clear').click(function()
        {
            ...
            var oTarget = $('#ec-product-searchdata-form .' + sSelectedFilterKey).parent();
            if (sKeyValue === 'price') {
                $('#ec-product-searchdata-form #ec-product-searchdata-minval').val('');
                $('#ec-product-searchdata-form #ec-product-searchdata-maxval').val('');
                EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMinPrice = 0;
    ...
    {
        $('#ec-product-searchdata-form  .ec-product-searchdata-price').parent().find('.xans-product-filterform  li').removeClass('selected');
        ...
        $('#ec-product-searchdata-form  #ec-product-searchdata-minval').val(SHOP_PRICE.toShopPrice(iMinPrice, false));
        $('#ec-product-searchdata-form  #ec-product-searchdata-maxval').val(SHOP_PRICE.toShopPrice(iMaxPrice, false));
    ...
    setSearchSubmit : function(sSortType, sFormName) {
        ...
        if ((sPathName === '/' || sPathName === '/index.html') || bSearchAll === 'T') {
            ...
            $("#ec-product-searchdata-form").append('<input type="hidden" name="search_all" value="T">');
        }

        $('#ec-product-searchdata-form').attr('action', sPathName);
    ...
    {
        if ($('#ec-product-searchdata-search-count').length  <= 0) {
            return;
        }
        if (this.bSearchSubmit === true) {
            return;
        }
        $('#ec-product-searchdata-form input:hidden[name^="search_form[option_data]"]').each(function() {
            $(this).remove();
        });
        ...
        $.post(sAction, $('#ec-product-searchdata-form').serialize(), function(aResultJson)
        {
            ...
            $('#ec-product-searchdata-submit').html(sMsg);
    ...
    setSearchLayerSubmit : function() {
        ...
        }).val('T').appendTo($('#ec-product-searchdata-searchkeyword_form'));
    ...
    setScrollCheck: function() {
        var oSearchdataArea = $('#searchSidebar #ec-searchdata-area');

        if (oSearchdataArea.length > 0) {
            oSearchdataArea.height($(window).height() - parseInt(oSearchdataArea.css('padding-top'), 10) - parseInt(oSearchdataArea.css('padding-bottom'), 10));
        }
    },
    ...
    setExtendButton : function()
    {
        ...
        if ($('.xans-product.xans-product-searchfilterlist .searchForm').length > 0) {
            $('.xans-product.xans-product-searchfilterlist .searchForm').find('.extend').hide();
        }
    },
    ...
    {
        if (oObjectSelector.attr('type') !== 'checkbox') {
    ...
    {
        $(oTargetForm + '.ec-product-searchdata-form.ec_search_selected').each(function() {
            if ($(this).attr('type') !== 'checkbox') {
                var sValueData = $(this).attr('sValue');
                if (sValueData !== undefined &&  sValueData !== ''  && EC_FRONT_PRODUCT_SEARCH_DATA.isDuplicateValueData(sValueData) === false) {
                    $('<input>').attr({type: 'hidden',name: 'search_form[option_data][]',value: encodeURIComponent(sValueData)}).appendTo('#ec-product-searchdata-form');
                }
            }
        });
    },
    ...
    setFacetFormParam : function(sFormName, sSortType) {
        ...
        if (iMinPriceValue !== 0) {
            $(sFormName + '#ec-product-searchdata-minval').val(iMinPriceValue);
        }

        if (iMaxPriceValue !== 0 ) {
            $(sFormName + '#ec-product-searchdata-maxval').val(iMaxPriceValue);
        }
        ...
        if ($('.withinInput input[type="text"]:visible').length > 0) {
            sWithin = $('.withinInput input[type="text"]:visible').val();
        } else {
            sWithin = $('.ec-product-searchdata-within').parent().find('.withinInput input[type="text"]').val();
        }
        ...
        if (typeof(sWithin) !== 'undefined') {
            ...
            if (sWithin.length > 0) {
                $('#ec-product-searchdata-form').append('<input type="hidden" name="within" value="' + sWithin + '"/>');
            }
        }
    },
...
var EC_FRONT_PRODUCT_SEARCH_DATA_SLIDE = {
    ...
    setInitPriceSet : function (eSearchPriceSlider, oTargetForm) {
        $(oTargetForm + '#eMinSlidePriceView').html(SHOP_PRICE_FORMAT.toShopPrice(this.iSearchSetMinPrice));
        $(oTargetForm + '#eMaxSlidePriceView').html(SHOP_PRICE_FORMAT.toShopPrice(this.iSearchSetMaxPrice));
        ...
        if (this.iSearchParamMinPrice > 0 || this.iSearchParamMaxPrice > 0) {
            ...
            $(oTargetForm + '#eMinSlidePriceView').html(SHOP_PRICE_FORMAT.toShopPrice(this.iSearchParamMinPrice));
            $(oTargetForm + '#eMaxSlidePriceView').html(SHOP_PRICE_FORMAT.toShopPrice(this.iSearchParamMaxPrice));
    ...
    setSearchClear : function () {
        ...
        if ($('#ec-product-fixed-slider').length > 0) {
            this.oCloneSlider.set([this.iSearchParamMinPrice, this.iSearchParamMaxPrice]);
        }

    },
    cloneSlider: function () {
        if (EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM.bFixedListForm === false || $('#ec-product-fixed-slider').length === 0) {
            return;
        }
...
var EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM = {
    ...
    setSearchFixListinit : function() {
        if ($('#ec-product-fixed-form .xans-product-fixedsearchfilterlist').length > 0 && $('#ec-product-fixed-form').length > 0) {
            this.bFixedListForm = true;

            // 패싯의 검색 엘리먼트 액션
            $('#ec-product-fixed-form .xans-product-fixedsearchfilterlist .ec-product-searchdata-form').click(function()
            {
                if (EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM.bLayerFormOpen === true) {
                    $('#ec-product-searchdata-form .xans-product-searchfilterlist').find('#'  + $(this).attr('id')).click();
                    return false;
                }

                EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM.setFixedFormSubmit($(this));
            });
            $('#ec-product-fixed-form .xans-product-fixedsearchfilterlist .ec-product-searchdata-price').click(function()
            {
                EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMinPrice = $(this).attr('min_price');
                EC_FRONT_PRODUCT_SEARCH_DATA.iSearchSetMaxPrice =  $(this).attr('max_price');
                EC_FRONT_PRODUCT_SEARCH_FIX_LIST_FORM.bFixedFormAction = true;
                EC_FRONT_PRODUCT_SEARCH_DATA.setSearchSubmit();
            });

            $('#ec-product-fixed-form .xans-product-fixedsearchfilterlist .ec-product-searchdata-form').each(function() {
                if ($(this).attr('type') === 'checkbox') {
                    var sObjectId = $(this).attr('id');

                    var oLabelObject = $('#ec-product-fixed-form .xans-product-fixedsearchfilterlist .ec-product-searchdata-form').find("label[for='" + sObjectId + "']");

                    if (oLabelObject.length > 0 ) {
                        oLabelObject.attr('id' , sObjectId + 'fixed');
                        $(this).attr('id' , sObjectId + 'fixed');
                    }
                }
            });
    ...
    setFixedFormData : function() {
        if (this.bFixedFormAction === true) {
            $('#ec-product-fixed-form .ec-product-searchdata-form:checked').each(function() {
                $('<input>').attr({type: 'hidden',name: 'search_form[option_data][]',value: encodeURIComponent($(this).val())}).appendTo('#ec-product-searchdata-form');
            });
        }
    } ,
    ...
    setLayerMoreFilter : function (bClose) {
        ...
        if (bClose === true) {
            $('#ec-product-fixed-form-filter').hide();
            this.bLayerFormOpen = false;
        } else {
            $('#ec-product-fixed-form-filter').show();

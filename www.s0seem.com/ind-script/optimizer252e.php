...
var FindId = function()
{
    ...
    this.action = function(sMallId , AuthModule)
    {
        ...
        var sCheckMethod = $('input[name="check_method"]:checked').val();
        ...
        var bResult = FwValidator.inspection('findIdForm');
        ...
        if (bResult == true) $('#findIdForm').submit();
 ...
 return false;//each 문에서는 break 안됨
    ...
    this.checkMobileAuth = function(){
        mobileAuthFindIdPopup( this.sMallId , this.AuthModule );
    };
    ...
    this.checkMobile = function()
    {
        ...
        $('input[name*="mobile"]').each(function(i, ele){
            if ($(this).val() == '') {
    ...
    /**
     * 입력폼 check
     * input 존재하지 않을 경우, 해당하는 element를 hide 합니다.
     */
    this.displayElementByInput = function()
    {
        var aInput = ['name', 'last_name'];
        $.each(aInput, function(i, val) {
            if ($('.xans-member').find('#'+val).length < 1 && $('.xans-member').find('#'+val+'_view').length > 0) {
                $('.xans-member').find('#'+val+'_view').hide();
            }
        });
    };
 ...
 *  ipinFindIdPopup
 */
function ipinFindIdPopup( sMallId )
{
    window.name = 'ipin_parent_window';
    window.open('', 'popupIpinfindid','width=448, height=500');

    var returnPort = location.protocol === 'https:' ? 443 : 80;

    //SSL 안타기 위해 joinForm 에서 보내지 않고 직접 만들어 보냄
    var sipinFindIdForm = '<form id="ipinFindIdForm" method="get" action="'+EC_FRONT_JS_CONFIG_MEMBER.sAuthUrl+'" class="testClass">';
    sipinFindIdForm += '<input type="hidden" name="service" value="echosting" />';
    sipinFindIdForm += '<input type="hidden" name="action" value="auth">';
    sipinFindIdForm += '<input type="hidden" name="authModule" value="nice" />';
    sipinFindIdForm += '<input type="hidden" name="authType" value="ipin" />';
    sipinFindIdForm += '<input type="hidden" name="method" value="GET" />';
    sipinFindIdForm += '<input type="hidden" name="mallId" value="'+sMallId+'" />';
    sipinFindIdForm += '<input type="hidden" name="mallVersion" value="shop19" />';
    sipinFindIdForm += '<input type="hidden" name="returnUrl" value="' + document.domain + '/exec/front/Member/IpinResult/" />';
    sipinFindIdForm += '<input type="hidden" name="returnPort" value="' + returnPort + '" />';
    sipinFindIdForm += '<input type="hidden" name="param1" value="lost_id" />';
    sipinFindIdForm += '<input type="hidden" name="param2" value="" />';
    sipinFindIdForm += '<input type="hidden" name="param3" value="" />';
    sipinFindIdForm += '</form>';

    if ( $('#ipinFindIdForm').html() == null ) $('body').append(sipinFindIdForm);
    
    $('#ipinFindIdForm').attr('target', 'popupIpinfindid');
    $('#ipinFindIdForm').submit();
}
 ...
 */
function mobileAuthFindIdPopup( sMallId, AuthModule )
{
    ...
    sMobileForm += '<input type="hidden" name="param1" value="lost_id" />';
...
$(document).ready(function(){
    ...
    try{
        ...
        if (document.findIdForm.searchType.length > 1) {
    ...
    // 입력폼 체크하여 element hide
    findId.displayElementByInput();

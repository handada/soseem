...
function allHidden() {
    ...
    if (document.getElementById('mobile_view') != null) {
...
function setFormType( type ) {
    ...
    try {
        var name_lable = document.getElementById('name_lable');
        var ssn_lable = document.getElementById('ssn_lable');

        var search_type_check = $($('input[name=check_method]').get(3));

        // 3번째 인증 수단의 값이 5( 휴대폰인증 ) 일 경우 아이핀은 사용되지 않는몰이다.
        if ( search_type_check.val() == '5' ) {
            var search_type_mobileauth = document.getElementById("check_method3");
        }

        // 3번째 인증 수단의 값이 4( 아이핀 인증 ) 일 경우 휴대폰 인증과 아이핀 둘다 할당 휴대폰인증이 없더라도 그냥 할당함. try catch로 예외처리
        if ( search_type_check.val() == '4' ) {
            var search_type_ipin = document.getElementById("check_method3");
            var search_type_mobileauth = document.getElementById("check_method4");
        }

        var search_type_ssn = document.getElementById("check_method0");
        var search_type_ipin_lable = document.getElementById("search_type_ipin_lable");
        var search_type_mobileauth_lable = document.getElementById("search_type_mauth_lable");

        var ssn_name = document.getElementById("ssn_name");

        switch (type) {

            case "corp" :
                name_lable.innerHTML = __('법인명');
                ssn_lable.innerHTML = __('법인번호');
                ssn_lable.parentNode.style.display = "inline";
                ssn_name.innerHTML = __('법인번호로 찾기');

                try{
                    search_type_ipin.style.display = 'none';
                    search_type_ipin_lable.style.display = 'none';
                }catch (e) {}

                try{
                    search_type_mobileauth.style.display = 'none';
                    search_type_mobileauth_lable.style.display = 'none';
                }catch (e) {}

                search_type_ssn.style.display = 'inline';
                ssn_lable.style.display = 'inline';
                find_typeLayer('2');
            break;
            ...
            case "indibuis" :
                ...
                // 유형관련 디자인 아예 손대지 않은 업체들의 경우
                try{
                    if (name_lable.innerHTML != 'First Name' && name_lable.innerHTML != '名') { //SUB-8402, SUB-10507
                        name_lable.innerHTML = __('이름');
                    }
                    ssn_lable.innerHTML = "";
                    ssn_name.innerHTML = "";
                }catch(e){}

                ssn_lable.style.display = 'none';
                ssn_lable.parentNode.style.display = "none";
                search_type_ssn.style.display = 'none';

                try{
                    search_type_ipin.style.display = 'inline-block';
                    search_type_ipin_lable.style.display = 'inline';
                }catch (e) {}

                try{
                    search_type_mobileauth.style.display = 'inline-block';
                    search_type_mobileauth_lable.style.display = 'inline';
                }catch (e) {}
            ...
            case "fore" :
                name_lable.innerHTML = __('이름');
                ssn_lable.innerHTML = __('등록번호');
                ssn_lable.parentNode.style.display = "inline";
                ssn_name.innerHTML = __('등록번호로 찾기');
                try{
                search_type_ipin.style.display = 'none';
                search_type_ipin_lable.style.display = 'none';
                }catch (e) {}
        ...
        var bCheck1 = $('.xans-member').find('#check_method1').click();
...
{    
     ...
     * submit main method
     */
    this.action = function(sMallId, AuthModule)
    {
        // 몰별 인증 건수 체킹을 위한 몰아이디 전송 
        this.sMallId = sMallId;
        this.AuthModule = AuthModule;

        var sCheckMethod = '';
        if (SHOP.getLanguage() == 'ko_KR') {
            sCheckMethod = $('input[name="check_method"]:checked').val();
        } else {
            sCheckMethod = '2';
        }

        var aFunctionMap = ['', 'SSN', 'Email',"Mobile",'Ipin','MobileAuth'];
        if (aFunctionMap[sCheckMethod] != 'SSN') {
            $('input[name*="ssn"]').each(function(i, ele){
                $(this).val('');
            });
        }

        var bResult = FwValidator.inspection('findPasswdForm');
        if (bResult.passed == false) return false;

        var sFunctionName = 'check' + aFunctionMap[sCheckMethod];
        
        var bResult = this[sFunctionName]();
         
        if (bResult == true) $('#findPasswdForm').submit();
        
        return false;
    };


    this.getNumberLable = function ( action )
    {
        switch ( action ) {
            case "indi" : return "주민등록번호"; break;
            case "corp" : return "법인등록번호"; break;
            case "fore" : return "외국인등록번호"; break;
        }
    };

    this.getNameLable = function ( action )
    {
        switch ( action ) {
            case "corp" : return __("법인명"); break;
            default : return __("이름"); break;
        }
    };
    ...
    {
        var bResult = true;
        var bSearchType = $('.xans-member').find('#searchType').val();
        var searchTypeName = this.getNameLable( bSearchType );
        var searchTypeNumber = this.getNumberLable( bSearchType );

        $('input[name*="name"]').each(function(i, ele){
            if ($(this).val() == '') {
                alert( sprintf(__('%s을 입력하세요.'), searchTypeName));
                ele.focus();
                bResult = false;
                return false;//each 문에서는 break 안됨
            }
        });
        ...
        $('input[name*="ssn"]').each(function(i, ele){
            if ($(this).val() == '') {
                alert( sprintf(__('%s를 입력하세요.'), searchTypeNumber));
                ele.focus();
                bResult = false;
                return false;//each 문에서는 break 안됨
            }
    ...
    this.checkEmail = function()
    {
        ...
        $("form#findPasswdForm :input[name='name']").each(function(){
            if ($(this).val() == '') {
                alert( sprintf(__('%s을 입력하세요.'), searchTypeName));
                $(this).focus();
                bResult = false;
     ...
     * Ipin validate
     */
    this.checkIpin = function()
    {
        ipinFindPwPopup( this.sMallId );
    };

    /**
     * 휴대폰 인증 validate
     */
    this.checkMobileAuth = function(){
        mobileAuthFindPwPopup( this.sMallId , this.AuthModule );
    };
    ...
    this.checkMobile = function()
    {
        ...
        $("form#findIdForm :input[name*='name']").each(function(){
            if ($(this).val() == '') {
                alert( sprintf(__('%s을 입력하세요.'), searchTypeName));
                ele.focus();
                bResult = false;
        ...
        $('.xans-member-findpasswd input[name*="mobile"]').each(function(i, ele){
            if ($(this).val() == '') {
                alert(__('휴대폰번호를 입력하세요.'));
                ele.focus();
                bResult = false;
                return false;//each 문에서는 break 안됨
            }
 ...
 *  ipinFindIdPopup
 */
function ipinFindPwPopup( sMallId )
{
    window.name = 'ipin_parent_window';
    window.open('', 'popupIpinfindpw','width=448, height=500');

    var returnPort = location.protocol === 'https:' ? 443 : 80;

    //SSL 안타기 위해 joinForm 에서 보내지 않고 직접 만들어 보냄
    var sipinFindPwForm = '<form id="ipinFindPwForm" method="get" action="'+EC_FRONT_JS_CONFIG_MEMBER.sAuthUrl+'" class="testClass">';
    sipinFindPwForm+= '<input type="hidden" name="service" value="echosting" />';
    sipinFindPwForm+= '<input type="hidden" name="action" value="auth">';
    sipinFindPwForm+= '<input type="hidden" name="authModule" value="nice" />';
    sipinFindPwForm+= '<input type="hidden" name="authType" value="ipin" />';
    sipinFindPwForm+= '<input type="hidden" name="method" value="GET" />';
    sipinFindPwForm+= '<input type="hidden" name="mallId" value="'+sMallId+'" />';
    sipinFindPwForm+= '<input type="hidden" name="mallVersion" value="shop19" />';
    sipinFindPwForm+= '<input type="hidden" name="returnUrl" value="' + document.domain + '/exec/front/Member/IpinResult/" />';
    sipinFindPwForm+= '<input type="hidden" id="returnPort" name="returnPort" value="' + returnPort + '" />';
    sipinFindPwForm+= '<input type="hidden" id="param1" name="param1" value="lost_pw" />';
    sipinFindPwForm+= '<input type="hidden" id="param2" name="param2" value="" />';
    sipinFindPwForm+= '<input type="hidden" id="param3" name="param3" value="" />';
    sipinFindPwForm+= '</form>';

    if ( $('#ipinFindPwForm').html() == null ) $('body').append(sipinFindPwForm);

    $('#ipinFindPwForm').attr('target', 'popupIpinfindpw');
    $('#param2').val( document.findPasswdForm.member_id.value );

    $('#ipinFindPwForm').submit();
}

/**
 *  mobileAuthFindIdPopup
 */
function mobileAuthFindPwPopup( sMallId, AuthModule )
{
    ...
    sMobileForm += '<input type="hidden" name="returnUrl" value="' + document.domain + '/exec/front/Member/MauthResult/" />';
    sMobileForm += '<input type="hidden" id="returnPort" name="returnPort" value="' + returnPort + '" />';
    sMobileForm += '<input type="hidden" id="param1" name="param1" value="lost_pw" />';
    sMobileForm += '<input type="hidden" id="param2" name="param2" value="" />';
    sMobileForm += '<input type="hidden" id="param3" name="param3" value="" />';
    sMobileForm += '</form>';
...
function fnMobilePopup() {
    ...
    var AUTH_POP = window.open('', popupName, winopts + position);
    document.forms['MauthForm'].target = popupName;
    document.forms['MauthForm'].submit();
}

findPasswd = new FindPasswd();


$(document).ready(function(){

    var bSearchType = $('.xans-member').find('#searchType');

    try{
        // 유형 추가 여부 체크 유형 추가 안된 몰의 경우 예외 처리 구간에서 주민번호 가림.
        if (document.findPasswdForm.searchType.length > 1) {
            if ($(bSearchType).val() == null) $(bSearchType).val('indi');
            setFormType( $(bSearchType).val() );
        }
    }
    catch(e){ setFormType( 'indi' ); }

    $(bSearchType).change(function() {
    ...
    var bCheck3 = $('.xans-member').find('#check_method2');
    var bCheck4 = $('.xans-member').find('#check_method3');
    var bCheck5 = $('.xans-member').find('#check_method4');

    var bCheck3Label = $('.xans-member').find('#search_type_mobile_lable');
    var mobile1 = $('.xans-member').find('#mobile1');
    
    if ( $('input[name*="mobile1"]').length < 1 ) {
        bCheck3.hide();
        bCheck3Label.hide();        
    }    
    ...
    $(bCheck1).click(function() {
        ...
        if ($(this).attr('checked') == true) {
            $('#email').val('');
            $('.xans-member').find('.mName').show();
            $('.xans-member').find('.ssn_no').show();
            $('.xans-member').find('.email').hide();
            $('.xans-member').find('.mobile').hide();
            $('.xans-member').find('.mobileauth').hide();                
            $('.xans-member').find('.ipin').hide();
        }
    });
    ...
    $(bCheck4).click(function() {
        ...
        if ($(this).attr('checked') == true) {
            ...
            $('#email').val('');
            $('#mName').val('');
            $('.xans-member').find('.mName').hide();
            $('.xans-member').find('.email').hide();
            $('.xans-member').find('.ssn_no').hide();
            $('.xans-member').find('.mobile').hide();

            // 상황에 따라서 4번째가 아이핀이 될수도 있고 휴대폰 인증이 될 수 도 있기 때문에 
            if ( $(this).val() == '4' ) {
               $('.xans-member').find('.mobileauth').hide(); 
               $('.xans-member').find('.ipin').show(); 
            }

            if ( $(this).val() == '5' ) {
               $('.xans-member').find('.mobileauth').show();
               $('.xans-member').find('.ipin').hide();
            }
        }     
    });

    $(bCheck5).click(function() {

...
var EC_ADDR_COMMONFORMAT = (function() {
    /**
    ...
    var aCountryDomainToShopLanguage = {
        'KR': 'ko_KR',
        'US': 'en_US',
        'JP': 'ja_JP',
        'CN': 'zh_CN',
    ...
    var init = function ()
    {
        ...
        if ($('#__isRuleBasedAddrForm').length > 0) {
            $('#__isRuleBasedAddrForm').val(isRuleBasedAddrFormVal);
        }
    ...
    var clearAddrField = function (sPageType)
    {
        ...
        for (var idx = 0; idx < aAddrFieldSelector.length; idx++) {
            ...
            var sAddrFieldSelector = $("#" + aAddrFieldSelector[idx]);
    ...
    {
        if (bIsInit) {
            var sIsRuleBasedAddrFormHiddenSelector = $('#__isRuleBasedAddrForm');
        ...
        if (aAddrInfo[sPageType]['aMarkupSettingData']['show_address'] === 'F') {
            ...
            for (var idx = 0; idx < aAddrInfo[sPageType]['aAllAddrFieldSelector'].length; idx++) {
                ...
                if (sSelector.indexOf('_wrap') === -1) {
                    $('#' + sSelector).attr('fw-filter', '');
                }
            }

            var sAddressWrapSelector = $('#ec-' + sPageType + '-address');
        ...
        for (var idx = 0; idx < aAddrInfo[sPageType]['aAllAddrFieldSelector'].length; idx++) {
            ...
            var sAddrFieldToHiddenSelector = $('#' + sAddrFieldToHidden);

            sAddrFieldToHiddenSelector.addClass('displaynone');
            sAddrFieldToHiddenSelector.hide();
            if (sAddrFieldToHiddenSelector.attr('type') === 'checkbox') {
                sAddrFieldToHiddenSelector.attr('checked', false);
            }
            if (sAddrFieldToHidden.indexOf('_wrap') === -1) {
                sAddrFieldToHiddenSelector.attr('disabled', true);
            }
        }
        ...
        for (var idx = 0; idx < aResultFields['aAddrOrdering'].length; idx++) {
            var sCurrentSelector = $('#' + aResultFields['aAddrOrdering'][idx]);
            var sNextSelector = $('#' + aResultFields['aAddrOrdering'][idx + 1]);
        ...
        for (var idx = 0; idx < aResultFields['aAddrFieldsToDisplay'].length; idx++) {
            var sDisplaySelector = $('#' + aResultFields['aAddrFieldsToDisplay'][idx]);
            if (sDisplaySelector.length > 0) {
                sDisplaySelector.removeClass('displaynone');
                sDisplaySelector.show();
                sDisplaySelector.removeAttr('disabled');
                sDisplaySelector.removeAttr('readonly');
            }
        }
    ...
    var getDisplayFieldsAndOrdering = function (sCountryCode, sPageType, aCountryRule)
    {
        ...
        if ($('#ec-' + sPageType + '-address').length > 0) aAddrFieldsToDisplay.push('ec-' + sPageType + '-address');
     ...
     * @param sCountryCode
     */
    var setAddrFieldName = function (sCountryCode)
    {
        ...
        for (var idx = 0; idx < aAddrOrdering.length; idx++) {
            var sHelpFieldMessageId = '';
            var sPlaceholderMessageId = '';
            var sWrapField = aAddrOrdering[idx];
            var sInputInWrapSelector = $('#' + sWrapField + ' > input');
            var sField = $('#' + sWrapField).attr('id').split('_')[1];
            ...
            if (sField === 'baseAddr') { /* 기본 주소 */
                ...
                if (sInputInWrapSelector.attr('fw-filter') !== 'isFill') {
                    sHelpFieldMessageId = 'OPTIONAL.' + sCountryCode;
                    sPlaceholderText += __(sHelpFieldMessageId, sGroupId);
                }
                ...
                var aAreaSelector = $('#' + sWrapField + ' > select');

                setSelectList(sCountryCode, 'city', $(aAreaSelector[1]));
                setSelectList(sCountryCode, 'street', $(aAreaSelector[2]));
    ...
    {
        if ($('#ec-shippingInfo-help').length > 0) {
            $('#ec-shippingInfo-help').insertAfter($('#' + sPageType + '_country_wrap'));
        }
    };
    ...
    var setConfigReadonly = function (aRuleFormat, sPageType)
    {
        ...
        if (isExistObj(aReadonlyFields)) {
            for (var i = 0; i < aReadonlyFields.length; i++) {
                var aReadonlySelector = $('#' + aAddrFieldSelector[aReadonlyFields[i]]);
                if (aReadonlySelector.length > 0) {
                    aReadonlySelector.attr('readonly', true);
                }
            }
        }
    };
    ...
    var setConfigChecked = function (aRuleFormat, sPageType)
    {
        ...
        if (isExistObj(aCheckedFields)) {
            for (var i = 0; i < aCheckedFields.length; i++) {
                var aReadonlySelector = $('#' + aAddrFieldSelector[aCheckedFields[i]]);
                if (aReadonlySelector.length > 0) {
                    aReadonlySelector.attr('checked', true);
                    /* 주문서 데이터 처리 */
        ...
        if (isExistObj(aMarkupSettingData) && aMarkupSettingData['uncheck_zipcode'] === 'T') {
            $("#" + aAddrFieldSelector['zipcodeCheck']).attr('checked', 'true');
        }
    };
    ...
    var setConfigDisabled = function (aRuleFormat, sPageType)
    {
        ...
        if (isExistObj(aDisabledFields)) {
            for (var i = 0; i < aDisabledFields.length; i++) {
                var aReadonlySelector = $('#' + aAddrFieldSelector[aDisabledFields[i]]);
                if (aReadonlySelector.length > 0) {
                    aReadonlySelector.attr('disabled', true);
                }
            }
        }
    };
    ...
    {
        var sZipcodeInputSelector = $("#" + aAddrInfo[sPageType].aAddrFieldSelector.zipcode);
        var sZipcodeCheckSelector = $("#" + aAddrInfo[sPageType].aAddrFieldSelector.zipcodeCheck);

        sZipcodeInputSelector.removeAttr('disabled');
        sZipcodeCheckSelector.removeAttr('checked');
    };
    ...
    var setConfigIsAreaAddr = function (sPageType, aRuleFormat)
    {
        ...
        EC_ADDR_COMMONFORMAT['aAddrInfo'][sPageType]['aIsAreaAddr'] = aIsAreaAddr;
    };
    ...
    var setConfigRequiredFields = function (sCountryCode, sPageType, aRuleFormat)
    {
        ...
        for (var i = 0; i < aRequiredFields.length; i++) {
            ...
            if (typeof (sSelectField) === 'string' && aRequiredFields[i] !== 'zipcode') {
                if (aDisplayFields.indexOf(sSelectField) > -1) $('#' + sSelectField).attr('fw-filter', 'isFill');
            } else { /* city, state, street */
                if (aIsAreaAddr.sIsAreaAddr === 'F') { /* 기본주소 검색이 select가 아님 */
                    if (sSelectField[sCountryCode] !== undefined) {
                        if (aDisplayFields.indexOf(sSelectField[sCountryCode]) > -1) $('#' + sSelectField[sCountryCode]).attr('fw-filter', 'isFill');
                    } else { /* 특정 국가의 필드가 있음 */
                        if (aDisplayFields.indexOf(sSelectField['DEFAULT']) > -1) $('#' + sSelectField['DEFAULT']).attr('fw-filter', 'isFill');
                    }
                } else { /* 기본주소 검색이 select 박스 */
                    if (aDisplayFields.indexOf(sSelectField['AREA']) > -1) $('#' + sSelectField['AREA']).attr('fw-filter', 'isFill');
                }
            }
        }
    };
    ...
    {
        if (isExistObj(common_aAddrInfo) === false || common_aAddrInfo['sIsRuleBasedAddrForm'] !== 'T' ||
            isExistObj(EC_ADDR_COMMONFORMAT['aAddrInfo'][sPageType]) === false ||
            isExistObj(EC_ADDR_COMMONFORMAT['aAddrInfo'][sPageType]['aIsAreaAddr']) === false
        ) {
            return false;
        }
        
        return EC_ADDR_COMMONFORMAT['aAddrInfo'][sPageType]['aIsAreaAddr'];
    };
    ...
    var changeCountryEvent = function (sCountryCode, sPageType, bIsInit)
    {
        if (bIsInit) {
            var aAddrFieldSelector = aAddrInfo[sPageType]['aAddrFieldSelector'];
            var sCountrySelector = $('#' + aAddrFieldSelector['country']);
    ...
    var clickZipcodeBtnEvent = function (sCountryCode, sPageType)
    {
        ...
        if ($(sZipcodeBtnSelector).length > 0 && aCountryDomainToShopLanguage[sCountryCode] !== undefined){
            /* 중복 이벤트 방지 unbind */
            $(sZipcodeBtnSelector + ' img').attr('src', $(sZipcodeBtnSelector + ' img').attr('off'));
            $(sZipcodeBtnSelector).unbind('click').css('cursor', 'unset');

            /* 우편번호 찾기 버튼 이벤트 바인딩 */
            $(sZipcodeBtnSelector + ' img').attr('src', $(sZipcodeBtnSelector + ' img').attr('on'));
            $(sZipcodeBtnSelector).bind('click', {
    ...
    var clickZipcodeCheckEvent = function (sPageType)
    {
        ...
        $(sZipcodeCheckSelector).unbind('click').css('cursor', 'unset');
        $(sZipcodeCheckSelector).bind('click', function() {
            if ($(this).is(':checked') === true) {
                $(sZipcodeSelector).val('');
                $(sZipcodeSelector).attr("disabled", true);
                if (typeof (EC_SHOP_FRONT_ORDERFORM_DATA) === 'object') EC_SHOP_FRONT_ORDERFORM_DATA.form.sIsNoReceiveZipcode = 'T';
            } else {
                $(sZipcodeSelector).removeAttr('disabled');
    ...
    var changeAreaStateEvent = function (sCountryCode, sPageType, bIsInit)
    {
        ...
        var sStateSelector = $('#' + aAddrFieldSelector['state']['AREA']);
        if (sStateSelector.length > 0 && aCountryDomainToShopLanguage[sCountryCode] !== undefined) {
            /* 바인딩 이벤트 최초 접근이 아니면 해당 메소드만 언바인딩 */
            if (!bIsInit) {
                sStateSelector.unbind('change.changeAreaState');
            }

            sStateSelector.bind('change.changeAreaState', function () {
                var aAddressSelectCallParams = {
                    sLanguage: aCountryDomainToShopLanguage[sCountryCode],
                ...
                if (bSetupResult === false) {
                    ...
                    $('#__city_name').val('');
                    $('#__addr1').val('');
                    $('#'+aAddrFieldSelector['baseAddr']).val('');
                }
            });
        }
    };
    ...
    var changeAreaCityEvent = function (sCountryCode, sPageType, bIsInit)
    {
        ...
        var sStateSelector = $('#' + aAddrFieldSelector['state']['AREA']);
        var sCitySelector = $('#' + aAddrFieldSelector['city']['AREA']);
        if (sCitySelector.length > 0 && aCountryDomainToShopLanguage[sCountryCode] !== undefined) {
            /* 바인딩 이벤트 최초 접근이 아니면 해당 메소드만 언바인딩 */
            if (!bIsInit) {
                sCitySelector.unbind('change.changeAreaCity');
            }

            sCitySelector.bind('change.changeAreaCity', function () {
                var aAddressSelectCallParams = {
                    sLanguage: aCountryDomainToShopLanguage[sCountryCode],
    ...
    var changeAreaStreetEvent = function (sCountryCode, sPageType, bIsInit)
    {
        ...
        var sStateSelector = $('#' + aAddrFieldSelector['state']['AREA']);
        var sCitySelector = $('#' + aAddrFieldSelector['city']['AREA']);
        var sStreetSelector = $('#' + aAddrFieldSelector['street']['AREA']);
        if (sStreetSelector.length > 0 && aCountryDomainToShopLanguage[sCountryCode] !== undefined) {
            /* 바인딩 이벤트 최초 접근이 아니면 해당 메소드만 언바인딩 */
            if (!bIsInit) {
                sStreetSelector.unbind('change.changeAreaStreet');
            }

            sStreetSelector.bind('change.changeAreaStreet', function () {
                var aAddressSelectCallParams = {
                    sLanguage: aCountryDomainToShopLanguage[sCountryCode],
                    si_name: sStateSelector.val(),
    ...
    var setUpNextAddrSelectbox = function(aAddressSelectCallParams, sCurrentField, aAddrFieldSelector, sCountryCode)
    {
        ...
        var sUrl = '/exec/front/zipcode/find/';
            ...
            success: function (response) {
                ...
                if (sCurrentField !== 'street') {
                    ...
                    if (sCurrentField === 'state' && aAddrFieldsToDisplay.indexOf(aAddrFieldSelector['city']['AREA']) > -1) {
                        sSelector = $('#' + aAddrFieldSelector['city']['AREA']);
                        sNextSelector = $('#' + aAddrFieldSelector['street']['AREA']);
                        sNextField = 'city';
                        sAfterNextField = 'street';
                    } else {
                        sSelector = $('#' + aAddrFieldSelector['street']['AREA']);
                        sNextField = 'street';
                    }
                    ...
                    sSelector = $('#' + aAddrFieldSelector['zipcode']);
    ...
    {
        var sAreaCitySelector = $('#' + aAddrFieldSelector['city']['AREA']);
        var sAreaStreetSelector = $('#' + aAddrFieldSelector['street']['AREA']);
    ...
    var setStateSelected = function(sCountryCode, sPageType, sStateHiddenData)
    {
        ...
        var aAddrFieldSelector = aAddrInfo[sPageType]['aAddrFieldSelector'];
        var sStateSelector = $('#' + aAddrFieldSelector['state'][sStateSelectorName]);
    ...
    var setAreaAddrSelected = function(sCountryCode, sPageType, aAreaHiddenData)
    {
        ...
        for( var sKey in aAreaHiddenData) {
            if (sKey === 'sStateName') {
                ...
                sSelector = $('#' + aAddrFieldSelector['state']['AREA']);
                sCurrentField = 'state';
    ...
    var setAddrHiddenBundle = function (sCountryCode, sPageType, aHiddenBundleData)
    {
        ...
        if (aHiddenBundleData === undefined) {
            sStateName = $('#' + aAddrFieldSelector['state']['AREA']).val();
            sCityName = $('#' + aAddrFieldSelector['city']['AREA']).val();
            sStreetName = $('#' + aAddrFieldSelector['street']['AREA']).val();
        } else {
        ...
        if (aIsAreaAddr['sIsThreeSelectInArea'] === 'T') { /* state, city, street 주소 검색 셀렉트박스가 모두 있는 국가 */
            if (sStateName == '' || sCityName == '' || sStreetName == '') {
                ...
                $('#__city_name').val(sCityName);
                $('#__addr1').val(sStreetName);
                $('#__address_addr1').val(sStreetName);

                var sDisplayText = makeDisplayText(sCountryCode, sStateName, sCityName, sStreetName);
                $('#' + sBaseAddrSelector).removeAttr('disabled');
                $('#' + sBaseAddrSelector).val(sDisplayText);

                /* 주문서 추가 처리 */
                if (aOrderPageType.indexOf(sPageType) > -1) {
                    $('#' + sStateInputSelector).removeAttr('disabled');
                    $('#' + sCityInputSelector).removeAttr('disabled');
                    $('#' + sStateInputSelector).val(sStateName);
                    $('#' + sCityInputSelector).val(sCityName);
                    $('#' + sBaseAddrSelector).val(sStreetName);
                    var aHiddenAddrInfo = new Array();
            ...
            if ( sStateName == '' || sStreetName == '') {
                ...
                $('#__state_name').val(sStateName);
                $('#__city_name').val(sStreetName);
                $('#' + sBaseAddrSelector).removeAttr('disabled');
                $('#' + sBaseAddrSelector).val(sStateName + ' ' + sStreetName);

                /* 주문서 추가 처리 */
                if (aOrderPageType.indexOf(sPageType) > -1) {
                    $('#__state_name').val('');
                    $('#__city_name').val('');
                    $('#' + sStateInputSelector).val('');
                    $('#' + sCityInputSelector).val('');
                ...
                if (aReceiverUpdatePageType.indexOf(sPageType) > -1) {
                    $('#__addr1').val($('#__state_name').val() + ' ' + $('#__city_name').val());
                }
            }
        }

        /* 상세주소 입력란으로 포커스 이동 */
        if ($('#' + aAddrFieldSelector['detailAddr']).length > 0) $('#' + aAddrFieldSelector['detailAddr']).focus();

        /* 지역별배송비 부과를 위해 event발생 */
        if (aMemberPageType.indexOf(sPageType) > -1) $('#' + aAddrFieldSelector['zipcode'] + ', #' + sBaseAddrSelector).blur();
    };
     ...
     * @param sStreetName
     */
    var makeDisplayText = function(sCountryCode, sStateName, sCityName, sStreetName)
    {
        var sDisplayText = '';

        var aDisplayTextList = common_aAddrInfo.aAllCountryFormat[sCountryCode].display_text;
        if (isExistObj(aDisplayTextList)) {
        ...
        sDisplayText = sStateName + ' ' + sCityName + ' ' + sStreetName;
        return sDisplayText;
    };
...
/**
 * 국가 도메인을 국가 코드로 변경하는 맵
 * - 기존에 국가코드를 3자리로 처리하는 곳에서 로직을 처리하기 위함
 */
var EC_ADDR_COUNTRY_CONVERT_MAP = (function() {
    var aCountryDomainToCodeMap = {
        'AW': 'ABW',
        'AF': 'AFG',
        'OM': 'OMN',
        'AO': 'AGO',
    ...
    return {
        aCountryDomainToCodeMap: aCountryDomainToCodeMap
    };
})();


$(document).ready(function() {
    EC_ADDR_COMMONFORMAT.init();

    // 주문 JS load 순서로 인하여, trigger 처리
    $(document).trigger('EC_ADDR_COMMONFORMAT_LOAD');
});

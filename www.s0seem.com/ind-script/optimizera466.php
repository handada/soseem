...
var OrderBoard = {
    ...
    selLimit : function(sFormName)
    {
        $('#'+sFormName).submit();
    },
    
    selProduct: function(product_no,iPrdImg, sPrdName,sPrdPrice, sPrdTaxText, sCategoryName, iCategoryNo, sOrderId)
    {
        //ECHOSTING-96629
        try {
            ...
            $('#order_id', opener.document).val(sOrderId);
            ...
            if ($('#sPrdTaxText', opener.document).length > 0) {
                $('#sPrdTaxText', opener.document).html(sPrdTaxText);
            }

            $('#iPrdView', opener.document).css('display', 'inline');
        } catch(e){}
        ...
        $('input[name^="fix_title_form_"]', opener.document).each(function (iIndex) {
            var sSubject = OrderBoard.getConvertString($(this).val(), sPrdName, sCategoryName);

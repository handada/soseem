        ...
        if (elmTarget.parents('tr').next().attr('id') == 'content_view') {
            ...
            $.get('/exec/front/board/Get/'+iBoardNo, aData, function(req) {
                if (req.failed == false) {
                    ...
                    if (rData.ucc_url) $('.ec-ucc-media-box-'+ rData.no).replaceWith(APP_BOARD_UCC.getPreviewElement(rData.ucc_url));
                } else {
...
$(document).ready(function(){
    $("#boardWriteForm").submit(function(e){
        return submitForm.write("boardWriteForm");
    });

    // 운영자이고..
    if (is_admin == 'T') {
        $(window).bind('beforeunload', function() {
            // 다음 상태가 처리해제 라는건 현 상태가 처리중.. 이라는 것..!!
            if ($('#eReplyStatusChangeTarget').val() == 'N') {
                return '다른 페이지로 이동하시면 답변상태가 처리중으로 남아있습니다.\n변경을 원하시면 처리해제를 눌러주세요.';
            }
        });

        // 본문에 focus 가도록 설정
        $('#content_IFRAME').contents().find('body').focus();
    }
    BOARD_WRITE.product_link();

    // 이벤트 발생시에만 게시안함 유무처리
    var $isPost = $("input:checkbox[name='is_post[]']");
    if ($isPost.length > 0) {
        $isPost.bind('click', function(e){
            if ($(this).is(":checked")) {
                $("#is_post_checked").val('B');
            } else {
                $("#is_post_checked").val('F');
            }
        });
    }
});
...
var submitForm = (function(){
    return {
        write : function ( formName ) {
            var pass = false,
                $form = $("#" + formName),
                valid = FwValidator.inspection(formName);
...
var BOARD_WRITE = {
    ...
    form_submit : function(sFormName)
    {
        ...
        var bPassed = submitForm.write("boardWriteForm");
        if (bPassed === false) {
            this.is_submitting = false;
            return;
        }
        
        if (parseInt($('#depth').val()) >= 10) {
            alert(__('답변을 등록할 수 없습니다. (답변 차수 제한)'));
             return false;
        }

        if ($('#privacy_agreement_radio0').length > 0) {
            var target = $('#privacy_agreement_radio0')[0].checked;
            if (target  === false) {
                alert(__('개인정보 수집 및 이용에 동의해 주시기 바랍니다.'));
                return false;
            }
        }

        if ($('#ucc').length > 0 && $('#ucc').val() != '') {
            if ((/^http[s]?:\/\/[a-z0-9_\-]+(\.[a-z0-9_\-]+)+/i).test($('#ucc').val()) === false) {
                alert(sprintf(__('%s 항목이 URL 형식이 아닙니다.'), 'UCC URL'));
                return false;
            }
        }

        if (bNameAuth === true && navigator.userAgent.match(/iPhone|iPod|Android/i) == null) {
            BOARD_WRITE.NameAuth();
            return false;
        }

        if (this.is_submitting === false) {
            this.is_submitting = true;

            var target = (this.currentTarget) ? this.currentTarget : this.srcElement; //ECHOSTING-106568
            $(target).hide();

            // 첨부파일 존재여부 체크 (ECHOSTING-158353)
            var bExistAttachFile = false;
            $('#boardWriteForm input[name="attach_file[]"]').each(function() {
                if ($(this).val().length > 0) {
                    bExistAttachFile = true;
                    return false;
                }
            });
            // 첨부파일이 없다면 form > enctype 기본 속성값으로 셋팅
            if (bExistAttachFile == false && $('input[name^="delfile"]:checked').length < 1) {
    ...
    product_popup : function (url)
    {
        window.open(url, 'pop_product' ,'width=540,height=400,scrollbars=yes');
    },
    NameAuth : function()
    {
        window.open('/protected/nameauth.html?ref=board', 'nameauth' ,'width=560,height=535, toolbar=0, menubar=0, scrollbars=0');
    },

    /**
     * 상품게시판의 등록/수정 페이지에서 링크가 비정상적인 상품thumnail이미지의 링크 무력화하기
     */
    product_link: function() {
        var $a = $('#iPrdImg').parent('a');
        if ($a.length) {
            $a.bind('click', function(e){
                var product_no = 0;
                var url = $(this).attr('href');
                $.each(url.substr(url.indexOf('?')+1).split('&'), function(i, str) {
                    if ('product_no' === str.replace(/\=[\d]+/ig, '')) {
                        product_no = parseInt(str.replace('product_no=', ''));
                        return;
                    }
                });

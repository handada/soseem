...
//필터 등록
FwValidator.Handler.setExtensionFilter('isOrderid', function(Response) {
    var sValue = '';
    var bResult = this.parent.CODE_FAIL;

    var value = Response.elmCurrValue;
    var aStr = value.split('-');

    if (aStr.length > 1) {
        var orderId1 = aStr[0];
        var orderId2 = aStr[1];
        if ( orderId1.match(/[^0-9]*/) != null && orderId2.match(/[^0-9]*/) ) {
            bResult = this.parent.CODE_SUCCESS;
        }
    }

    return this.parent.Helper.getResult(Response, bResult);
});

//에러메세지 등록
FwValidator.Handler.setFilterErrorMsg('isOrderid', __('ENTER.NUMBER.HYPHENS', 'APP.FORM.RESOURCE.JS'));
